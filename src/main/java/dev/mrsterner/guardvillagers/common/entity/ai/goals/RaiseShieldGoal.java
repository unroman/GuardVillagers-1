...
public class RaiseShieldGoal extends Goal {
    ...
    @Override
    public void stop() {
        if (false/*TODO !GuardVillagersConfig.get().GuardAlwaysShield*/)
            guard.stopUsingItem();
    }
    ...
    protected boolean raiseShield() {
        ...
        if (target != null && guard.shieldCoolDown == 0) {
            ...
            return guard.distanceTo(target) <= 4.0D || target instanceof CreeperEntity || target instanceof RangedAttackMob && target.distanceTo(guard) >= 5.0D && !ranged || target instanceof RavagerEntity || true/*TODO || GuardVillagersConfig.get().GuardAlwaysShield*/;
        }
        return false;
    }
}