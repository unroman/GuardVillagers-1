...
public class WalkBackToCheckPointGoal extends Goal {
    ...
    public boolean canStart() {
        return guard.getPatrolPos() != null && !this.guard.getPatrolPos().isWithinDistance(
                new Vec3i(
                        this.guard.getPos().getX(),
                        this.guard.getPos().getY(),
                        this.guard.getPos().getZ()), 1.0D) && !guard.isFollowing() && guard.isPatrolling();
    }
    ...
    public void tick() {
        ...
        if (blockpos != null) {
            ...
            if (!this.guard.getPatrolPos().isWithinDistance(new Vec3i(
                    this.guard.getPos().getX(),
                    this.guard.getPos().getY(),
                    this.guard.getPos().getZ()), 1.0D) || blockpos != null)
